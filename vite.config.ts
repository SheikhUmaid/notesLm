...
const config = () => {
  return defineConfig({
    ...
    // base: '/chitchatter/',
    server: {
      allowedHosts: ['.localhost', '.localdomain','qsj37lhk-3000.inc1.devtunnels.ms', 'inc1.devtunnels.ms'],
      proxy: {
        '/api': {
      ...
      https: {
        key: fileURLToPath(
          new URL('key.pem', import.meta.url)
        ),
        cert: fileURLToPath(new URL('cert.pem', import.meta.url)),
      },
    },
